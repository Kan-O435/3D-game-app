...
- `src/ui/` — DOM-rendered React screens overlaid on the `<Canvas>` (sibling `<div>`s), styled as a slot cabinet (palette + shared classes in `src/index.css`: `.slot-panel`, `.led-window`/`.led`, `.lamp`, `.marquee`/`.bulb`, `.arcade-btn`, `.win-banner`, `.neon`). One per game status: `SpinButton` (title/playing), `OrderSheet` (briefing), `Hud` (LED coin/performance windows + one lamp per remaining turn) + `PayoutTable` + `Marquee` (chasing-bulb stage sign) + `Callouts` (WIN / BIG WIN / JACKPOT / RATE LIMIT / drift banners) (playing), `Shop` (shop), `GameOverScreen`; plus `MuteButton`, `Hotkeys` (Space/Enter = the obvious action for the current screen, 1–8 buy in the shop) and the small helpers `LedNumber` (rolling number, `countUp.ts`) and `winTier.ts`. Overlays that follow a camera move use the `.fade-in` class (delayed so they appear once the camera has settled). All read `src/store` only.
